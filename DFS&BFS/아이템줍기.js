...
function solution(rectangle, characterX, characterY, itemX, itemY) {
  ...
  const visited = new Set(); // 좌표를 저장하고 has로 찾는다.
  ...
  function 재귀(newRectangle, x, y) {
    ...
    function execution(i) {
      if (x === rectangle[i][0]) {
        //up
        visited.add(`${(x + direction.up[0], y + direction.up[1])}`);

        재귀(rectangle[i], x + direction.up[0], y + direction.up[1]);
        //down
        visited.add(`${(x + direction.down[0], y + direction.down[1])}`);

        재귀(rectangle[i], x + direction.down[0], y + direction.down[1]);
        //어느 면인지 확인.
      }
      if (y === rectangle[i][3]) {
        //left
        visited.add(`${(x + direction.left[0], y + direction.left[1])}`);

        재귀(rectangle[i], x + direction.left[0], y + direction.left[1]);
        //right
        visited.add(`${(x + direction.right[0], y + direction.right[1])}`);
